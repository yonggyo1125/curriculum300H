...
- 액티비티는 시스템에서 관리하지만 <b>프래그먼트는 액티비티 위에 올라가 있어 액티비티를 전환하지 않고도 훨씬 가볍게 화면 전환 효과를 만들 수 있게 됩니다. 특히탭 모양으로 화면을 구성할 때 각각의 [탭] 버튼을 클릭할 때마다 다른 화면이 보이는 효과를 내고 싶다면 액티비티가 아닌 프래그먼트를 사용하는 것이 좋습니다.
