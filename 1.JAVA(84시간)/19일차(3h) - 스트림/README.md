...
- Optional<T>은 제네릭 클래스로 'T 타입의 객체'를 감싸는 래퍼 클래스이다.
- Optional타입의 객체에서는 모든 타입의 참조변수를 담을 수 있다.
```
public final class Optional<T> {
...
```
- 최종 연산의 결과를 그냥 반환하는 게 아니라 Optional 객체에 담아서 반환하는 것 
- 객체에 담아서 반환을 하면, 반환된 결과가 null인지 매번 if문으로 체크하는 대신 Optional에 정의된 메서드를 통해 간단히 처리할 수 있다.
