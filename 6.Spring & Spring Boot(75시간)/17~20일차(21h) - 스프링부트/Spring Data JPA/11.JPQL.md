...
> <b>글로벌 로딩 전략</b><br>- 엔티티에 직접 적용하는 로딩 전략 -> 애플리케이션 전체에 영향을 미침<br> 예) <code>@OneToMany(fetch = FetchType.LAZY)<br>즉시 로딩으로 설정 시 성능에 악영향 미칠 수 있음<br>- 애플리케이션 전체에서 항상 즉시 로딩 발생<br>- 사용하지 않는 엔티티를 자주 로딩하게 됨 
...
><b>META-INF/orm.xml</b><br>JPA가 기본 매핑파일로 인식 → 별도 설정 필요 없음<br>이름이나 위치가 다를 경우 설정 추가 필요
